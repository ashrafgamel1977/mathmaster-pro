...
const MENU_GROUPS = [
  ...
  { 
    ...
    items: [AppView.STUDENTS, AppView.MANAGEMENT, AppView.SCHEDULE, AppView.STUDENT_PORTAL] 
  },
  { 
    id: 'content', 
    label: 'المحتوى والاختبارات', 
    icon: <BookOpen size={18} />,
    items: [AppView.COURSES, AppView.ASSIGNMENTS, AppView.QUESTION_BANK, AppView.QUIZZES, AppView.FILES, AppView.FORMULAS] 
  },
  { 
    id: 'tools', 
...
const DEFAULT_LABELS: Record<string, string> = {
  ...
  [AppView.QUESTION_BANK]: 'بنك الأسئلة'
};

const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, settings, loggedUser, pendingCount = 0, unreadChatCount = 0, unreadNotificationCount = 0, pendingStudentCount = 0, isConnected = false, onLogout }) => {
  ...
  const renderItem = (id: string, labelOverride?: string) => {
    ...
    if (id === AppView.NOTIFICATIONS) badge = unreadNotificationCount;
