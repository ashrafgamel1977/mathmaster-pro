      ...
      <div className={`bg-black w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl relative animate-slideUp flex flex-col h-[85vh] border-8 transition-all duration-300 ${getBorderColor()}`}>
        ...
        <div className="absolute top-0 left-0 right-0 z-30 p-4 bg-gradient-to-b from-black/90 to-transparent">
           ...
           <div className="relative mt-2">
             <label htmlFor="camera-select" className="sr-only">اختر الكاميرا</label>
             <select 
               id="camera-select"
               aria-label="اختر الكاميرا"
               className="w-full p-3 pl-10 bg-slate-800/90 backdrop-blur-md border border-slate-600 rounded-xl font-bold text-xs md:text-sm text-white outline-none appearance-none cursor-pointer hover:bg-slate-700 transition-all shadow-lg text-right"
               value={selectedCameraId}
               onChange={(e) => setSelectedCameraId(e.target.value)}
               disabled={cameras.length === 0}
             >
               {cameras.length === 0 && <option value="">جاري البحث عن كاميرات...</option>}
               {cameras.map(cam => (
                 <option key={cam.id} value={cam.id}>{cam.label || `كاميرا ${cam.id.slice(0,5)}`}</option>
               ))}
             </select>
