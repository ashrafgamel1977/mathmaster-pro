...
import { QuizResult, Student, AppNotification, PlatformSettings } from '../types';
import { generateQuizReportPDF } from '../services/reportingService';
import { sendQuizResultToParent } from '../services/whatsappService';
...
const QuizResults: React.FC<QuizResultsProps> = ({ results, students, notifications, onIssueCertificate, onUpdateResult, notation, settings }) => {
  ...
  const handleSendQuickReport = async (studentName: string, parentPhone: string, quizTitle: string, score: number) => {
    const res = await sendQuizResultToParent(settings, studentName, parentPhone, quizTitle, score);
    if (res.ok && res.mode !== 'link') {
      ...
      <div className="bg-white p-10 md:p-14 rounded-[4rem] shadow-xl border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex gap-3">
           <div className="flex bg-slate-100 p-1 rounded-2xl">
            ...
            <button 
              onClick={exportToCSV}
              className="px-6 py-3 bg-slate-100 text-slate-600 rounded-2xl font-black text-xs hover:bg-slate-200 transition-all flex items-center gap-2"
              title="تصدير CSV"
            >
              <span className="text-lg">📊</span>
            </button>
            <button 
              onClick={() => generateQuizReportPDF(results, students, settings, 'كشف الدرجات العام')}
              className="px-6 py-3 bg-red-600 text-white rounded-2xl font-black text-xs shadow-xl shadow-red-100 hover:scale-105 transition-all flex items-center gap-2"
            >
              <span>تصدير PDF</span>
              <span className="text-lg">📄</span>
            </button>
