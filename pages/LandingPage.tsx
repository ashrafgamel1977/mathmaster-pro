...
interface LandingPageProps {
  ...
  students?: Student[];
  onUnifiedLogin: (role: 'student' | 'teacher' | 'parent', code: string, students?: Student[], settings?: PlatformSettings) => Promise<boolean>;
...
const LandingPage: React.FC<LandingPageProps> = ({ teacherName, platformName, settings, students = [], onUnifiedLogin, onStudentRegister }) => {
  ...
  const { isAllowed, blockMessage, recordFailure, recordSuccess } = useRateLimiter();
  ...
  const handleSubmit = async () => {
    if (!credential || !isAllowed || isLoggingIn) return;
    setIsLoggingIn(true);

    // Execute login and capture result
    const success = await onUnifiedLogin(activeRole, credential, students, settings);
          ...
          <div className="bg-[#1e293b]/90 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-2xl border border-white/10 relative overflow-hidden group">
            ...
            <div className="space-y-8 relative z-10">
              ...
              <div className="grid grid-cols-3 gap-2 bg-[#0f172a] p-1.5 rounded-2xl border border-white/5">
                {[
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center py-3 rounded-xl transition-all duration-300 ${activeRole === role.id
              ...
              <div className="space-y-5">
                ...
                {/* Rate Limit Warning */}
                {!isAllowed && (
                  <div className="bg-rose-500/10 border border-rose-500/30 rounded-2xl px-4 py-3 text-center">
                    <p className="text-rose-400 font-bold text-sm">{blockMessage}</p>
                  </div>
                )}

                <button
                  onClick={handleSubmit}
                  disabled={!isAllowed || isLoggingIn}
                  className={`w-full py-5 rounded-2xl font-black text-lg shadow-xl transition-all relative overflow-hidden group ${isAllowed && !isLoggingIn
                      ? 'shadow-orange-600/20 hover:shadow-orange-600/40 hover:scale-[1.02] active:scale-95'
                ...
                >
                  ...
                  <span className="relative text-white flex items-center justify-center gap-2">
                    {isLoggingIn ? 'جاري التحقق...' : (isAllowed ? 'تسجيل الدخول' : '🔒 محظور مؤقتاً')}
                    {isAllowed && !isLoggingIn && <span className="text-xl">➔</span>}
                  </span>
                </button>
              </div>
